...
export async function writeMessageWithFiles(
  ...
  for (const file of files) {
    ...
    const ok = await ingestOneFile(baseUrl, writeToken, payload, fileBuffer, file, mode);
    if (ok) ingested++;
  }
  ...
  return writeOneMessage(baseUrl, writeToken, payload, mode);
}
  ...
  for (const { payload, attachmentCount, files } of payloads) {
    ...
    if (attachmentCount > 0 && files && files.length > 0) {
      ...
      if (ingestedAny) {
        ...
        outcome = await writeOneMessage(baseUrl, writeToken, payload, mode);
      }
